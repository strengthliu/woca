...
void SharedMatting::loadImage(cv::Mat inputMat)
{
	pImg = new IplImage(inputMat);
	if (!pImg)
	{
...
void SharedMatting::loadTrimap(cv::Mat inputTrimap)
{
	trimap = new IplImage(inputTrimap);
	if (!trimap)
	{
...
cv::Mat SharedMatting::saveContourMask(int ksize){//added by jacques
	cv::Mat result(matte,1);
